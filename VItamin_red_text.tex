...
\begin{table}
\centering
{\color{red}\caption{Durations required to produce samples from each of
...
& min & max & median & \\
\hline
\texttt{Dynesty}\footnote{\new{The benchmark samplers all produced
$\mathcal{O}(10000)$ samples dependent on the default sampling parameters
used.}}~\cite{dynesty} & 11795 & 29838 & 19400
\footnote{\new{We note that there are a growing number of specialised
...
\texttt{VItamin}\footnote{\new{For the \texttt{VItamin} sampler $10000$ samples are
...
specifically from Will Farr, Tom Dent, Jonah Kanner, and the parameter estimation 
and machine-learning working groups. We would additionally like to thank Szabi 
Marka for posing this challenge to us \new{and the journal referees for their 
helpful and contstructive comments}. We thank Nvidia for the generous donation 
...
\begin{table}
\centering
{\color{red}\caption{The uniform prior boundaries and fixed parameter values used on the \ac{BBH} signal parameters for the benchmark
...
mass 2 & $m_2$\footnote{\new{Additionally $m_2$ is constrained such that
...
\begin{table*}
    ...
    flatten\footnote{\new{Take the multi-channel output of the previous layer and
    reshape it into a one-dimensional vector.}}$\rightarrow$[4224] & flatten$\rightarrow$[4224] \\
    & & append\footnote{\new{Append the argument to the current dataset.}}($z$)$\rightarrow$[4234] & append($x$)$\rightarrow$[4231] \\
    \hline
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \multirow{3}{*}{Layer 4} & 
    \multirow{3}{*}{
    ...
    \multirow{3}{*}{
    \begin{tabular}[t]{c}
    FC(4234,2048)\footnote{\new{Fully
    connected layer with arguments (input size, output size).}}\\
    dropout(0.2)\footnote{\new{Drop-out layer with argument (drop-out
    fraction).}} \\
    act=ReLU \\
    \end{tabular}
    } &
    ...
    \begin{tabular}[t]{c}
    FC(2048,14) \\
    act=(Sigmoid,-ReLU)\footnote{\new{Different activations are used for different
    parameters. For the scaled parameter means we use
    sigmoids and for log-variances we use negative ReLU functions.}} \\
    output=$\mu_{r_2}$ \\
    $\rightarrow$[7,2]\footnote{\new{The $r_{\theta_2}$ output has size [physical space dimension, No. parameters defining
    ...
    act=None \\
    output=$\mu_{q}$ \\
    $\rightarrow$[10,2]\footnote{\new{The $q_{\phi}$ output has size [latent space
    ...
    act=None \\
    output=$\mu_{r_1}$ \\
    $\rightarrow$[10,16,2]\footnote{\new{The $r_{\theta_1}$ output has size
...
%
\new{The $r_{\theta_1}$ network takes the input time-series data $y$ in the form of
...
Gaussian Process optimisation algorithm choses a new set of network
...
\begin{enumerate}
%
\item \new{The encoder $q_{\phi}$ takes both the time-series
...
%
\item \new{The encoder $r_{\theta_1}$ is given only the time-series data
...
\begin{table}
\centering
{\color{red}\caption{Benchmark sampler configuration parameters. Values were chosen based
