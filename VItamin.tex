...
distributions on $\phi_{12},\phi_{jl}$,  we model $\psi, \phi$ as locations in an 
abstract 2D plane\hunter{Added small modification here to address phase/psi},  and truncated Gaussians for
...
both tests the quality of the \texttt{VItamin} results are consistent with the
...
update and the phase/psi reparameterization trick.  Would you like to write this bit here Chris?}
...
\begin{figure*}
    ...
    of source parameter. In each box plot we show the distribution of 1-dimensional 
     ...
     (blue). The lower and upper end of boxes represent the 25th and 75th credibility
      ...
      and 95th percent credibility regions. 
...
then compute the \ac{JS}-divergence between the single \ac{GW} source parameter 
output distributions from each
sampler with the output distributions from the \texttt{Dynesty} sampler~\cite{4839047}.  
For distributions that are
identical, the \ac{JS}-divergence \textit{should} equal zero but since we are
...
\ac{JS}-divergences for the 250 test \ac{GW} samples as a function 
of \ac{GW} source parameter.  We see that the \ac{JS}-divergence distribution
of \texttt{Dynesty} vs. \texttt{emcee} contains the highest \ac{JS}-divergence values 
out of all samplers, whereas \texttt{Dynesty} vs.  \texttt{ptemcee} seems to 
show the most similarity between posterior distributions.  It is also shown that 
\texttt{VItamin} vs. \texttt{Dynesty} \ac{JS}-divergence distributions generally 
lie within similar levels of disagreement as those from both \texttt{CPNest} vs. 
\texttt{Dynesty} and \texttt{ptemcee} vs. \texttt{Dynesty}.  \hunter{I added my own 
interpretation of the results here as an addition.}
