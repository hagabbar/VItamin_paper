...
% TO-DO LIST
%
% 1. Get abstract word count roughly correct - currently 240
% 2. Get total word count roughly correct - currently (195, 680, 960, 620) =
% 2455
% 3. Clean captions and check word count
% 4. Fix all plots
% 5. Tidy Methods section
% 6. Polish language throughout
% 7. Check acronymns for consistency
...
found in the letter text.}  % word count 152 
\end{figure}
...
%both black hole masses $m_1$ and $m_2$, which we allow to range accross
%$[35,80]$ solar masses, a uniform phase ($\phi_0$) prior spanning $[0,2\pi]$
%radians, a uniform distance ($d_{\text{L}})$ prior ~\chris{we might want to
%change this to be a $d$-squared prior to demonstrate the application of priors
%in the \texttt{VItamin} training stage.} between $[1,3]$~Gpc, and a uniform
%time of coalescence ($t_{\text{c}}$) prior ranging between
%$[0.65,0.85]$~seconds from the start of the $1$ second duration data segment.
%The remainder of the parameters are set to the following fixed values: sky
%position ($\alpha=?,\beta=?$), binary inclination angle ($\iota=0$),
%polarisation angle ($\psi=0$), and black hole spins set to zero).
...
logarithmically for readability.} % word count 85 
\end{figure}
%
...
unique test samples and $3000$ posterior sample predictions per test sample.
...
defined as the total time (in seconds) to produce $\sim 3000$ samples for each
...
\begin{table}
\centering
\caption{Durations required to produce $3000$ posterior samples from each of
...
dependent upon the mass of binary. We have chosen to use the noticably low
...
conditional latent variable model and our parameteric model is constructed from
the product of 2 seperate distributions marginalised over the latent space
%
...
training session over one batch of signals.} % word count 73
\end{figure}
...
and back-propogate cost function derivatives to update the network parameters.
...
%
% Describe the specific network hyperparameters
%
...
use a latent space dimension of $64$ and we consider training complete when
