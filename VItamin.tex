...
defined \new{in Eq.~\ref{eq:approx_post}.} We have used $\theta_{1}$ and
...
%
\new{TODO: describe each encoder and decoder network structure. Describe the
decoder specific output distributions. Mention the hyper-parameter optimisation}
...
conv\_1d(32,7,3) & conv\_1d(32,7,3) & conv\_1d(32,7,3) \\
ReLU & ReLU & ReLU \\
dropout(0.2) & dropout(0.2) & dropout(0.2) \\ 
conv\_1d(32,7,16) & conv\_1d(32,7,16) & conv\_1d(32,7,16) \\
ReLU & ReLU & ReLU \\
dropout(0.2) & dropout(0.2) & dropout(0.2) \\
conv\_1d(32,7) & conv\_1d(32,7) & conv\_1d(32,7) \\
ReLU & ReLU & ReLU \\
conv\_1d(32,7) & FC(2048) & FC(2048) \\
conv\_1d(32,7) & FC(2048) & FC(2048) \\
FC(2048) & $\mu_{r_2}=\left\{\begin{array}{c}
\mu \\
\log(\sigma^2) \end{array}\right.$ & $\mu_{q}=\left\{\begin{array}{c}
\mu \\
\log(\sigma^2) \end{array}\right.$ \\
FC(2048) & & \\
$\mu_{r_1}=\left\{\begin{array}{c}
\text{mode weights} \\
\mu \\
\log(\sigma^2) \end{array}\right.$ & & \\
\botrule
...
We have used $\theta_{1}$ and
$\theta_{2}$ to indicate that the 2 separate networks modelling these
distributions will be trained on these parameter sets respectively. \new{The
encoder $r_{\theta_1}(z|y)$ takes as inout the data $y$ and outputs $M$ sets of
means ond variances of $n_{z}$-dimensional multivariate
Gaussian distributions together with a length-$M$ vector of component weights. These
outputs then describe the parameters of a $M$-component $n_{z}$-dimensional Gaussian mixture
distribution within the latent space. The decoder $r_{\theta_2}(x|z,y)$ takes
as input a single location within the latent space together with the data $y$
and outputs sets of parameters describing specific distributions on subsets of
the physical paramters $x$.}


conditional distributions are modelled as $n_{z}$ dimensional multivariate
uncorrelated Gaussian distributions (governed only by their means and variances).
However, this still allows $r_{\theta}(x|y)$ to take a general form (although
it does limit it to be unimodal).
...
%
\new{TODO: move the learning rate stuff here. Also discuss the ramp}
...
sampling method are (0.211,0.972 VItamin), (0.832,0.043 dynesty), (0.117,0.728
ptemcee), (0.840,0.189 cpnest), (0.002,0.489 emcee). 
}
\end{figure}
%
...
\hline
Dynesty~\cite{dynesty} & 5000 & 0.1 & N/A & N/A & N/A \\
Emcee~\cite{emcee} & N/A & N/A & 250 & N/A & 14000 \\
Ptemcee~\cite{ptemcee} & N/A & N/A & 250 & 8 & 5000 \\
Cpnest~\cite{cpnest} & 5000 & 0.1 & N/A & N/A & N/A \\
\botrule
\end{tabular}
