...
layer and has a latent space dimension of $64$. Training is considered complete when both components to the loss
...
%~\chris{Can you try to use the white space in the top
...
The estimated future detections for \ac{BNS} $\sim 180$ will swamp our current computational resources using 
...
%calculation and the tolerance on that result}. 
The sampler takes $\mathcal{O}(3
\: \textrm{minutes})$ to converge
%~\chris{jargon. What does converge mean to the
%average Nature reader?}. 
After the nested sampler has converged
%~\chris{jargon}
, we draw
samples
%~\chris{too technical with not enough explanation.
% The \texttt{Bilby} analysis
%produces posterior parameter estimates and does so by supplying the %user with
%samples drawn from that distribution.}
 and produce a posterior on the
