...
test signal was simulated with the optimal multi-detector signal-to-noise ratio of
17.2.}} 
\end{figure*}
...
EPSRC grant EP/M01326X/1, and RM-S EPSRC grants EP/M01326X/1, EP/T00097X/1 and EP/R018634/1. 
...
act=None \\
output=$\mu_{q}$ \\
$\rightarrow$[2,10]\footnote{The $q_{\phi}$ output has size [latent space
dimensionm, No. parameters defining the distribution per dimension].} \\
\end{tabular}
} \\
& & & \\
& & & \\
...
act=None \\
output=$\mu_{r_1}$ \\
$\rightarrow$[10,12,2]\footnote{The $r_{\theta_1}$ output has size
...
together with the $M=12$ mixture component weights. The motivation for using this
...
compound time-series data ``feature-space'' and true signal parameters that are
...
$n_z=10$ means and log-covariances. The rationale behind this choice is that
...
particular time-series belongs to.}      
