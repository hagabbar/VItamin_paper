...
~\hunter{Update to include the GWTC-2 and GWTC-2.1}
%
% rationale
%
...
%
\hunter{2D reparameterisation is on all 4 parameters}
\hunter{Need to change the parameters to be in words "parameters with cyclic 
prior bounds"}
...
$\phi_0$. We internally  For each parameter we use a uniform prior with the exception of 
...
\hunter{internally typo }
\hunter{don't need an for spin parameters}
\hunter{we internally marginalise can go}
\hunter{mention mass conditional prior, we also enforce m1>m2}
\hunter{Fix phase marginalisation error here. Needs to be 14 parameters.}
%
% the VItamin process
%
...
\hunter{Could get rid of this big vector of parameters}
\hunter{On the 15 dimensional parameters X, comparison results do 
not use phi0 when comparing to Bayesian samplers.}
\hunter{make last sentence one sentence from since. don't need "of the 
samplers" at the end.}
\hunter{Move this sentence to the end of the previous paragraph.}
%
% Corner plot results
%
\begin{figure*}
...
expert recommended sampler settings \hunter{add reference to sampler settings 
table}.  For each of our 
...
\hunter{Maybe this last sentence should be cut}
...
both tests the quality of the \texttt{VItamin} results are~\hunter{I changed this.}
...
\hunter{fix space typo in fig.2}
...
and GW190425~\cite{2020ApJ...892L...3A}) and \ac{NSBH} systems\hunter{reference 
NSBH paper} where
...
introducing the \ac{KL} cost term contribution via an annealing process.\hunter{
Add the definition of an epoch somewhere (if not already there)}} 
\end{figure}
...
\begin{table*}
\centering
\caption{The \texttt{VItamin} network hyper-parameters \hunter{for the 
2d abstract plane each parameter, is actually two. So 19,2 in r2. Cyclic 
parameters have two means and two variances since it's in the 2d plane.
Make a statement that says they're shared and use dash in r2 and q. State in 
beginning of text of network design that this is the case.}}
...
\hline
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\multirow{2}{*}{Layer 5} & conv(16,96,96) & conv(16,96,96) &
conv(16,96,96) \\
& L2Reg(0.001) & L2Reg(0.001) & L2Reg(0.001) \\
& act=LeakyReLU & act=LeakyReLU & act=LeakyReLU \\
\hline
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\multirow{2}{*}{Layer 6} & conv(16,96,96) & conv(16,96,96) &
conv(16,96,96) \\
& stride(2) & stride(2) & stride(2) \\
& L2Reg(0.001) & L2Reg(0.001) & L2Reg(0.001) \\
& act=LeakyReLU & act=LeakyReLU & act=LeakyReLU \\
...
output=$\mu_{r_2}$ \\
$\rightarrow$[15,2]\footnote{The $r_{\theta_2}$ output has size [physical space dimension, No. parameters defining
...
\hunter{May want to mention 2D representation here.}
...
set of posterior samples. \hunter{don't have fractions in text.}

\hunter{psi and phi are not together cyclic, so thus we must reparameterise them
.}
\hunter{The reason the 2D plane is used is because there's no bounderies. also, very 
easy to jump from 0 to 2pi}
\hunter{Have to say at the end of this. This parametersation is completey acceptable
for spinning, precessing waveforms, but not appropriate when considering higher order 
modes.}
...
likelihoods. \hunter{change to truncated, see thesis.}
...
\hunter{change epoch definition.}
\hunter{Add one sentence. We augment the data on the amplitude, phase and time 
of arrival every time we load in new data. This is done by shifting all the 
parameters randomly when we read it in.}
...
\hunter{Add reference to github code when mentioning vitamin at first. (page 2}
%
%\begin{figure*}
...
identical, the \ac{JS}-divergence {should} \hunter{results in finiti 
JS divergence values. Remove the end of this sentence.} equal zero but since we 
