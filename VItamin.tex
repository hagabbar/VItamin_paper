...
$\theta_{1},\theta_{2},\phi$. These parameters are the weights and biases of 3
neural networks acting as (variational) encoder, decoder, and encoder
...
\begin{itemize}
%
\item The encoder $r_{\theta_1}$ is given a batch of training \ac{GW} signals
($y$) and encodes them into a set of variables $\mu_{r_1}$ defining a
...
Ptemcee~\cite{ptemcee} & walkers $=250$ temperatures $=8$ steps $=5000$ \\
Cpnest~\cite{cpnest} & live-points $=5000$ tolerance $=0.1$ \\
Emcee~\cite{emcee} & walkers $=250$ steps $=14000$ \\
\botrule
\end{tabular}
