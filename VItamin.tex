...
priors in the VItamin training stage.} from $1$---$3$~Gpc, and a time of
coalescence ($t_{\text{c}}$) prior ranging from
$0.7$---$0.9$~seconds~\chris{please verify and change these numbers} from the
...
including Vitamin, is shown in Table~\ref{Tab:speed}. Run-time is defined as
the total time to produce $\sim 5000$ samples for each sampler in seconds. It
can be clearly seen that Vitamin produces samples from the posterior at a rate
...
configuration. We choose Bayesian sampler hyperparameters based on standard the
standard default configurations defined in the \texttt{Bilby}
documentation~\cite{1811.02042}~\chris{this is noce and should be used as our
defence for all things regarding Bilby.}.~\chris{You need to add more about the
time spent training our analysis just to cover our backs. We can't be seen to
be hiding this. Also, we need to add more samplers to this list.} 
