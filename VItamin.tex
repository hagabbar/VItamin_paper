...
GW signals $y$. We take the output from encoder-1 $\mu^{j}_1$ 
and sample from a multivariate Gaussian distribution described by 
$\mu^{j}_1$ in order to get 
a set of $z_{1}$ samples. Our $z_{1}$ samples are then combined with our 
test GW signals $y_{\textrm{test}}$ and fed as input to our pre-trained decoder 
network. The decoder network returns a set of mean $\mu^{1}_x$ predictions 
from which we sample from a multivariate Gaussian distribution in order to get 
